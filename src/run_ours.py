...
from process_CASIAv2 import Forgery_Dataset, Patch
import copy
...
from model_utils import Forgery_Detector
from model_backbone_ours import CNN_Fusion as backbone
from model_CAFE_ours import CNN_Fusion as cafe
from model_BMR_ours import CNN_Fusion as bmr
from model_MCAN_ours import CNN_Fusion as mcan
from model_SAFE_ours import CNN_Fusion as safe
...
def main(args):
    ...
    if args.baseline == 'backbone':
        ...
        model = cafe(args)
    forgery_detector = Forgery_Detector()
    print('loading data...')
    ...
    train_loader = DataLoader(dataset=train_dataset, batch_size=args.batch_size, num_workers=args.number_workers,
                              shuffle=True, drop_last=True)
    validate_loader = DataLoader(dataset=validate_dataset, batch_size=args.batch_size, num_workers=args.number_workers,
                                 shuffle=False)
    test_loader = DataLoader(dataset=test_dataset, batch_size=args.batch_size, num_workers=args.number_workers,
                             shuffle=False)
    forgery_loader = DataLoader(dataset=forgery_dataset, batch_size=args.batch_size, num_workers=args.number_workers,
                                shuffle=True, drop_last=True)
    forgery_iter = iter(forgery_loader)

    if torch.cuda.is_available():
        print("CUDA")
        model.cuda()
        forgery_detector.cuda()
    patch = Patch()
    ...
    optimizer_forgery = torch.optim.Adam(forgery_detector.parameters(), lr=args.forgery_lr, eps=args.adam_epsilon)

    logger.info('begin pre-training forgery detector...')
    for epoch in range(args.pretrain_epochs):
        for i, data_forgery in tqdm.tqdm(enumerate(forgery_loader)):
            forgery_images, forgery_labels = to_var(data_forgery[0]), to_var(data_forgery[1]).squeeze()
            optimizer_forgery.zero_grad()
            forgery_detector_output, _ = forgery_detector(forgery_images)
            loss_training_forgery = criterion(forgery_detector_output, forgery_labels)
            loss_training_forgery.backward()
            optimizer_forgery.step()
        logger.info("teacher loss: {:.4f}".format(loss_training_forgery.item()))
    ...
    for epoch in range(args.num_epochs):
        ...
        for i, (train_data, train_labels, event_labels) in tqdm.tqdm(enumerate(train_loader)):
            train_text, train_image, train_mask, train_labels, event_labels = to_var(train_data[0]), to_var(
                train_data[1]), to_var(train_data[2]), to_var(train_labels), to_var(event_labels)
            try:
                data_forgery = next(forgery_iter)
            except:
                forgery_iter = iter(forgery_loader)
                data_forgery = next(forgery_iter)
            forgery_images, forgery_labels = to_var(data_forgery[0]), to_var(data_forgery[1]).squeeze()
            ...
            # directly optimizing the forgery teacher, au - 0, tampered- 1
            forgery_detector_output, _ = forgery_detector(forgery_images)
            image_patched = patch.patching(train_image)
            train_image_forgery_output, _ = forgery_detector(train_image)  # unlabeled
            train_image_patched_forgery_output, _ = forgery_detector(image_patched)  # positive
            loss_pu_forgery = torch.log(torch.sum(train_image_forgery_output[:, 1])) - torch.log(torch.tensor(train_image.shape[0])) - torch.sum(train_image_patched_forgery_output[:, 1]) / train_image.shape[0]
            loss_pu_forgery = loss_pu_forgery.sigmoid()
            if not (1 > loss_pu_forgery > 0):  # when all samples in one batch are real, loss_pu = NaN
                loss_training_forgery = criterion(forgery_detector_output, forgery_labels)
            else:
                loss_training_forgery = criterion(forgery_detector_output,
                                                  forgery_labels) + args.delta * loss_pu_forgery
            loss_training_forgery.backward()
            optimizer_forgery.step()

            if args.baseline == 'safe':
                class_outputs, _, image_z, text_z, sim, forgery_pred, intent_pred, _, _ = model(train_text, train_image, train_mask)
                loss_sup = criterion(class_outputs, train_labels) + criterion(sim, train_labels)
            else:
                class_outputs, _, image_z, text_z, forgery_pred, intent_pred, _, _ = model(train_text, train_image, train_mask)
                loss_sup = criterion(class_outputs, train_labels)

            # forgery part
            forgery_detector_output, _ = forgery_detector(train_image)
            # hard distillation
            # _, argmax = torch.max(forgery_detector_output, 1)
            # loss_kl = criterion(forgery_pred, argmax)
            # soft distillation
            loss_kl = torch.nn.functional.kl_div(forgery_pred.softmax(dim=1).log(), forgery_detector_output.softmax(dim=1))

            # intent part, MMD: 0 - real, 1 - fake; INT: 0 - positive, 1 - negative
            train_labels_unlabeled = to_var(torch.ones(train_labels.shape[0])) - train_labels
            intent_pred_phi = intent_pred.softmax(-1)[:, 0]
            """
                please refer to "A Variational Approach for Learning from Positive and Unlabeled Data" (Alg.1) for the specific formula.
                train_labels - 0: real -> positive, 1: fake -> negative/unlabeled
                intent_pred_phi - the probability of positive prediction
            """
            loss_pu = torch.log(torch.sum(train_labels * intent_pred_phi)) - torch.log(torch.sum(train_labels)) - \
                      torch.sum(train_labels_unlabeled * torch.log(intent_pred_phi)) / torch.sum(train_labels_unlabeled)
            loss_pu = loss_pu.sigmoid()  # to avoid the loss < 0
            if not (1 > loss_pu > 0):  # when all samples in one batch are real, loss_pu = NaN
                loss = loss_sup + args.alpha * loss_kl + args.gamma * contrastiveLoss(image_z, text_z)
            else:
                loss = loss_sup + args.alpha * loss_kl + args.beta * loss_pu + args.gamma * contrastiveLoss(image_z, text_z)
            loss.backward()
            optimizer.step()
        ...
        for i, (validate_data, validate_labels, event_labels) in enumerate(validate_loader):
            validate_text, validate_image, validate_mask, validate_labels, event_labels = to_var(
                validate_data[0]), to_var(validate_data[1]), to_var(validate_data[2]), to_var(validate_labels), to_var(
                event_labels)
            if args.baseline == 'safe':
                validate_outputs, _, _, _, _, _, _, _, _ = model(validate_text, validate_image, validate_mask)
            else:
                validate_outputs, _, _, _, _, _, _, _ = model(validate_text, validate_image, validate_mask)
        ...
        # model.train()
        logger.info(
            'Epoch [%d/%d], Loss: %.4f, KL loss: %.4f, PU loss: %.4f, Teacher loss: %.4f, Train_Acc: %.4f, Validate_Acc: %.4f, Validate_F1: : %.4f.' %
            (epoch + 1, args.num_epochs, np.mean(cost_vector), loss_kl.item(), loss_pu.item(),
             loss_training_forgery.item(), np.mean(acc_vector), validate_accuracy, validate_f1))
    ...
    test_unimodal_pred = []
    test_true = []
    final_emb_list, forgery_emb_list, intent_emb_list = [], [], []
    for i, (test_data, test_labels, event_labels) in enumerate(test_loader):
        ...
        if args.baseline == 'safe':
            test_outputs, final_emb, _, _, _, _, _, forgery_emb, intent_emb = model(test_text, test_image, test_mask)
        else:
            test_outputs, final_emb, _, _, _, _, forgery_emb, intent_emb = model(test_text, test_image, test_mask)
        final_emb_list.append(final_emb.detach().clone().to('cpu'))
        forgery_emb_list.append(forgery_emb.detach().clone().to('cpu'))
        intent_emb_list.append(intent_emb.detach().clone().to('cpu'))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--delta', type=float, default=0.1, help='training forgery teacher part')
    parser.add_argument('--alpha', type=float, default=0.1, help='training forgery part')
    parser.add_argument('--beta', type=float, default=0.1, help='training intent part')
    ...
    parser.add_argument('--forgery_lr', type=float, default=0.0002, help='')
