...
class CNN_Fusion(nn.Module):
    ...
    def forward(self, text, image, mask):
        ...
        # OURS
        forgery_z = self.forgery_extractor(image_atn_feature)
        forgery_pred = self.forgery_classifier(forgery_z)
        text_image_forgery = torch.cat((text_atn_feature, image_atn_feature, forgery_z), 1)
        intent_z = self.intent_extractor(text_image_forgery)
        intent_pred = self.intent_classifier(intent_z)
        final = torch.cat((final_feature_main_task_lite, forgery_z, intent_z), dim=1)

        mix_output = self.mix_classifier(final)
        mix_output = self.dropout(mix_output)
        return mix_output, final_feature_main_task_lite, image_z, text_z, forgery_pred, intent_pred
