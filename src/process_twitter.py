...
def write_data(flag, image, text_only):
    ...
    def read_post(flag):
        ...
        if flag == "train":
            ...
            images = [image.split('.')[0].lower() for image in os.listdir(pre_path + 'images_train')]
        elif flag == "validate":
            texts = open(pre_path + 'train_posts.txt', 'r').readlines()
            images = [image.split('.')[0].lower() for image in os.listdir(pre_path + 'images_validation')]
        elif flag == "test":
            texts = open(pre_path + 'test_posts.txt', 'r').readlines()
            images = [image.split('.')[0].lower() for image in os.listdir(pre_path + 'images_test')]
        else:
            print('Error')
            return
        ...
        for i, line in enumerate(texts):
            ...
            text = text_dict[post_id]  # TODO: clean?
            # text = line.split('\t')[1]

            continue_flag = False
            for image in image_id:
                if image.strip() in images:
...
def get_data(text_only):
    ...
    validate_data, _ = write_data("test", image_list, text_only)
